...
This section demonstrates the use of transactions to manage updates in the database, ensuring data integrity through rollback and commit operations.
...
- **Step 1: Initial State (Before Update)**:  
  The initial state of the `Lease` table for LeaseID 400 is shown before any changes are made.  
  ![Rollback Step 1 - Initial State](./images/R1.png)

- **Step 2: After Update (Before Rollback)**:  
  The transaction begins, and the update is applied, increasing the `DiscountPercent` by 5 for LeaseID 400. This step shows the temporary state of the database after the update but before the rollback.  
  ![Rollback Step 2 - After Update](./images/R2.png)

- **Step 3: After Rollback (Returns to Initial State)**:  
  The transaction is rolled back, reverting the database to its initial state. This step confirms that the `DiscountPercent` for LeaseID 400 has returned to its original value, as the update was not committed.  
  ![Rollback Step 3 - After Rollback](./images/R3.png)

### Update with COMMIT
**Story**: Following a room renovation, the housing office decided to permanently increase the maximum number of occupants for a specific room (RoomID = 400) by 1, changing the `MaxOccupants` from 3 to 4. They performed the update within a transaction and committed it to ensure the change is saved, confirming the update's success. Afterward, they attempted a rollback to verify that the change was indeed permanent.

This demonstrates an update to the `Room` table with a commit, showing the database state at each step:

- **Step 1: Initial State (Before Update)**:  
  The initial state of the `Room` table for RoomID 400 is shown, with `MaxOccupants` set to 3 before any changes are made.  
  ![Commit Step 1 - Initial State](./images/C1.png)

- **Step 2: After Update (Before Commit)**:  
  The transaction begins, and the update is applied, increasing the `MaxOccupants` by 1 for RoomID 400 (from 3 to 4). This step shows the temporary state of the database after the update but before the commit.  
  ![Commit Step 2 - After Update](./images/C2.png)

- **Step 3: After Commit (Change is Permanent)**:  
  The transaction is committed, making the update permanent. This step shows that the `MaxOccupants` for RoomID 400 remains 4, confirming the successful commit.  
  ![Commit Step 3 - After Commit](./images/C3.png)

- **Step 4: Attempt to Rollback After Commit (No Effect)**:  
  An attempt to rollback is made after the commit, but since the transaction was already committed, the database state does not change. This step confirms that the `MaxOccupants` for RoomID 400 is still 4, as the commit made the update permanent.  
  ![Commit Step 4 - Rollback Attempt](./images/C4.png)

---

## Final SQL Files
The final SQL files for this phase are included in the repository:
